function G_causal = I_SCAP_I(G_connection,G_causal)
SE = 1;
SF = 2;
C = 3;
I = 4;
...
for i=1:length(I_nodes)
    ...
    if ((edgecount(G_causal,I_neighbor_node,I_nodes(i))==0) && (edgecount(G_causal,I_nodes(i),I_neighbor_node)==0))%Do not add edges if an edge edge already exists
        G_causal = addedge(G_causal,I_neighbor_node,I_nodes(i));
